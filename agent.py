...
def _build_session(tools: list, system_prompt: str) -> AgentSession:
    ...
    if _google_llm and _deepgram_stt and _google_tts:
        ...
        return AgentSession(
            stt=_deepgram_stt,
            llm=_google_llm.with_temperature(0.7),
            tts=_google_tts.with_voice(voice_name),
            tools=tools,
        )

    raise RuntimeError("No valid AI configuration found. Install livekit-plugins-google and livekit-plugins-deepgram")
...
async def entrypoint(ctx: agents.JobContext) -> None:
    await _log("info", f"Agent job started: {ctx.job.metadata}")

...
async def get_all_settings() -> dict:
    ...
    # Supabase settings table is only used for display, never for actual configuration
    KNOWN_KEYS = [
    ...
    out: dict = {}
    for k in KNOWN_KEYS:
        env_val = os.getenv(k, "")  # Direct environment variable access ONLY
...
async def get_setting(key: str, default: str = "") -> str:
    # VPS Environment Variables are SINGLE SOURCE OF TRUTH
    # Never check Supabase settings table for actual values
...
async def log_call(
    ...
    db = await _adb()
    # Convert to IST timezone (Asia/Kolkata = UTC+5:30)
